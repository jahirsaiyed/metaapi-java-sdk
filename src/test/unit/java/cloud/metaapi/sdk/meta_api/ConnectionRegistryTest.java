...
class ConnectionRegistryTest {
    ...
    void setUp() {
        ...
        Mockito.when(metaApiWebsocketClient.subscribe(Mockito.anyString()))
    ...
    void testReturnsTheSameConnectionOnSecondConnectIfSameAccountId() throws IllegalAccessException  {
        ...
        Mockito.verify(metaApiWebsocketClient).addSynchronizationListener("id0", storage);
        Mockito.verify(metaApiWebsocketClient).addSynchronizationListener("id1", storage);
        Mockito.verify(metaApiWebsocketClient).subscribe("id0");
        Mockito.verify(metaApiWebsocketClient).subscribe("id1");
        Mockito.verify(metaApiWebsocketClient, Mockito.times(2)).subscribe(Mockito.anyString());
        @SuppressWarnings("unchecked")
        Map<String, MetaApiConnection> registryConnections = (Map<String, MetaApiConnection>) FieldUtils
            .readField(registry, "connections", true);
