    ...
    public CompletableFuture<Void> onSymbolPriceUpdated(MetatraderSymbolPrice price) {
        ...
        if (specification.isPresent()) {
            for (MetatraderPosition position : positions) {
                ...
                if (position.unrealizedProfit.isEmpty() || position.realizedProfit.isEmpty()) {
                    position.unrealizedProfit = Optional.of(
                    ...
                    position.realizedProfit = Optional.of(position.profit - position.unrealizedProfit.get());
                }
                ...
                double unrealizedProfit = (position.type == PositionType.POSITION_TYPE_BUY ? 1 : -1) *
                    ...
                    position.volume / specification.get().tickSize;
                double increment = unrealizedProfit - position.unrealizedProfit.get();
                position.unrealizedProfit = Optional.of(unrealizedProfit);
                position.profit = position.unrealizedProfit.get() + position.realizedProfit.get();
